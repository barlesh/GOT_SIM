...
%% API
-export([start_link/1, start_biggles/2, remote_close_char/2, move_request/2, multimedia_update/2, stop/2]).
...
%%--------------------------------------------------------------------
%% @doct
%% Request permision of char to move to other biggle 
%%
%% @spec move_request	 -> {ok, Pid} | ignore | {error, Error}
%% @end
%%--------------------------------------------------------------------
move_request(Own_pid, {Destination, Type, From_pid, Ref}) ->
	gen_server:cast(Own_pid, {move_request, Destination, Type, From_pid, Ref}).

multimedia_update(Own_pid, {Msg}) ->
	gen_server:cast(Own_pid, {multimedia_update, Msg}).
...
%	biggle_gen_server:create_world( biggle_1, lists:nth(1, List_per_biggle) ), 	
	...
	{reply, ok , State#state{board_size=Board_size}};
...
handle_cast({move_request, Destination, Type, From_pid, Ref}, State)->
	Which_biggle=where_is_location(Destination, State), 	
	Answer=biggle_gen_server:remote_move_request(Which_biggle, {Destination, Type}), 		
	biggle_gen_server:move_request_ans(From_pid, {Ref, Answer}), 		
	io:format("master: move request to ~p, in ~p, Answer: ~p, ~n",[Destination, Which_biggle, Answer]), 	
	{noreply, State};

handle_cast({multimedia_update, Msg}, State)->
...
handle_cast(Msg, State) ->
	io:format("biggle_master handle cast ?? ~p ~p~n", [Msg, State]),    
	{noreply, State}.
...
return_remote_chars(Location, Radius, From_pid, State)->
	...
	X<-lists:seq(1,State#state.num_biggles), lists:nth(X, State#state.biggle_list)/=From_pid  ],
...
where_is_location({X,Y}, State)->
	io:format("master: where is location: {~p,~p}~n",[X,Y]),
	Low_lim=round( (State#state.board_size) /2 ),
	Up_lim=State#state.board_size,
	Big1=(X>0 andalso X<Low_lim andalso Y>0 andalso Y<Low_lim), 
	Big2=(X>Low_lim andalso X<Up_lim andalso Y>0 andalso Y<Low_lim), 
	Big3=(X>0 andalso X<Low_lim andalso Y>Low_lim andalso Y<Up_lim),	
	Big4=(X>Low_lim andalso X<Up_lim andalso Y>Low_lim andalso Y<Up_lim), 
		
	case {Big1,Big2,Big3,Big4} of 
		{true,false,false,false}-> {global,biggle_1};
		{false,true,false,false}-> {global,biggle_2};
		{false,false,true,false}-> {global,biggle_3};
		{false,false,false,true}-> {global,biggle_4};
		_Else->
			io:format("biggle master, where is location, error~n"),
			error  
	end.
