...
-record(state, {biggle, board_size, type, location, pid, radius=20, step_size=80, attackee, attacker, bodies, waiting_req=[]}).
							 %attacker=pid, bodies for ww. 
-record(request,   {type, ref, waiting_4, from}). %type =   
%%%===================================================================
%%% API
%%%===================================================================
...
idle(timeout, State) ->	
	case (State#state.waiting_req /= []) of
		true->
			io:format("Idle/2: Pid ~p, Type: ~p, timed out on cast reply ~p.~n",[State#state.pid, State#state.type,
			State#state.waiting_req]);
		_Else->
			do_nothing
	end,
	{next_state, waking, State#state{ waiting_req=[] }, 0};

idle({cast_ans, Request_ref, Answer}, State)->
	io:format("Idle/2: Pid ~p, Type: ~p, got cast ans. Answer:~p ~n",[State#state.pid, State#state.type, Answer]),
	{Next_State, New_state, Timeout}=manage_cast_ans(Request_ref, Answer , State), 
...
idle({attacked, Other_pid}, State)->
	...
	{next_state, attacked, State#state{attacker=Other_pid, waiting_req=[]}, 0};
...
waking(timeout, State) ->
	...
	case Close_chars of
		{wait, Request_rec} ->
			io:format("~p ~p, in waking, recieved {wait, ~p}~n",[State#state.type, State#state.pid, Request_rec]),		
			New_waiting_req=State#state.waiting_req++[Request_rec],
			New_state_rec=State#state{waiting_req=New_waiting_req}, 
			{Next_state, New_state_rec, Timeout}={idle, New_state_rec, timeout(idle)}; 
		_Else->
			Close_list=tuple_to_list(list_to_tuple(Close_chars)),
			io:format("Type: ~p, Pid: ~p, Location: ~p, close chars: ~p~n", 
			[State#state.type,State#state.pid,State#state.location, Close_list]),
...
%Called when receiving a delayed answer from biggle_server. Returns {Next_State, New_state, Timeout}
manage_cast_ans(Request_reference, Answer, State)->
	io:format("~p ~p in manage_cast ans, waiting_req=~p~n",[State#state.type, State#state.pid, State#state.waiting_req]),	
	Waiting_req=[X||X<-State#state.waiting_req, Request_reference=:=X#request.ref], 		
	%Check if request exists in State.waiting_req and if not go to idle. 
	case (Waiting_req) of
	[]->
		io:format("~p ~p in manage_cast_ans - no matching request.~n",[State#state.type, State#state.pid]), 
		{idle, State, timeout(idle)};
	_Else->%Reference_found!
		io:format("~p ~p in manage_cast_ans - found ref - ~p.~n",[State#state.type, State#state.pid, Waiting_req]), 
		case (lists:nth(1,Waiting_req))#request.type of
			move_request ->
				case Answer of
					{remote_move_approved, {}}->
						io:format("char manage cast - remote approved, should recieve stop");
					{remote_move_denied, {}}  ->
						io:format("char manage cast, ~p ~p remote move denied.~n",[State#state.type, 							State#state.pid]);
					_Else ->
						io:format("char manage cast - remote move now answer match, answer: ~p~n", [Answer])	
				end, 
				Req_list=State#state.waiting_req--Waiting_req, 
				{Next_state, New_state_rec, Timeout}={idle,State#state{waiting_req=Req_list}, timeout(idle)}; 
			get_close_chars -> 				
				Close_list=Answer,
				io:format("In manage cast: Type: ~p, Pid: ~p, Location: ~p, close chars: ~p~n", 
				[State#state.type,State#state.pid,State#state.location, Close_list]),
				%io:format("Type: ~p, Pid: ~p, Action: ~p~n", [State#state.type,State#state.pid, Action]),	
				Action=choose_action(State#state.type, State#state.board_size, Close_list, State#state.location, 					State#state.step_size), 
				{Next_state, New_state_rec, Timeout}=preform_action(State#state{waiting_req=[]}, Action) 
		end  
	end.
	
				
	%If so deal according to kind: 
	%1) Move_request - preform_action(State, {complete_move, {Reply, Destination}}) ->
	%2) get_close_chars	
...
preform_action(State, {move, {Destination}}) ->
	...
	case (Reply) of
		...
		_Else ->
			io:format("performing action move, No case match for ~p###~n",[Reply]),
			New_state_rec=State, 
			Next_state=idle 
	end, 	 
	{Next_state, New_state_rec, timeout(Next_state)};

%If received deleyed answer from biggle_server, is irrelevent 
preform_action(State, {finish_move, Answer}) ->
	{Reply, Destination}=Answer, 
	case (Reply) of
		'ok' ->
			New_state_rec=State#state{location=Destination};  
		'denied' ->
...
preform_action(State, {complete_move, {Reply, Destination}}) ->
	case (Reply) of
		'ok' ->
			New_state_rec=State#state{location=Destination, waiting_req=[]}, 
			Next_state=idle;
		'denied' ->
			New_state_rec=State#state{ waiting_req=[]}, 
			Next_state=idle; 
		{'wait', Request_rec}  ->
			Next_state=idle, 
			New_state_rec=State, 			
			io:format("received wait when completeing action, not leagal.%%%%%~n")		
	end, 	 
	{Next_state, New_state_rec, timeout(Next_state)};
...
get_destination(Board_size, Close_chars, {X0,Y0}, Step_size)->
	...
	case (X1>0 andalso Y1>0 andalso X1<Board_size andalso Y1<Board_size 
	     andalso X1/=(round(Board_size)/2) andalso Y1/=(round(Board_size)/2)) of 
		false ->
			get_destination(Board_size, Close_chars, {X0,Y0}, Step_size); 
		true  ->
...
choose_action(white_walker, Board_size, Close_chars, Location, Step_size) ->
	Bodies = [],%trans[ {Etype, Location1, Pid} || {Etype, Location1, Pid}<-Close_chars, Etype=:=body], 
	Enemys = [],%trans[ {Etype, Location1, Pid} || {Etype, Location1, Pid}<-Close_chars, Etype=:=warrior],	
...
choose_action(Type, Board_size, Close_chars, Location, Step_size) ->%Warrior or zombie
	...
	case Type of 
		warrior ->
			Enemys=[];%trans[ {Etype, Location1, Pid} || {Etype, Location1, Pid}<-Close_chars, Etype=:=zombie orelse Etype=:=white_walker];
		zombie  ->
			Enemys=[]%trans[ {Etype, Location1, Pid} || {Etype, Location1, Pid}<-Close_chars, Etype=:=warrior]	
	end, 
	case (Enemys) of	 
